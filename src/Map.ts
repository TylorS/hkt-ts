...
/**
 * Gets `AssociativeIdentity` instance for Maps given `Associative` instance for their values
 *
 */
export function makeAssociativeIdentity<K, A>(
  SK: Eq<K>,
  SA: Associative<A>,
): AssociativeIdentity<ReadonlyMap<K, A>> {
  const lookupWithKeyS = lookupWithKey(SK)
  return {
    concat: (mx, my) => {
      if (isEmpty(mx)) {
...
export const makeUnionAssociativeIdentity = <K, A>(
  E: Eq<K>,
  S: Associative<A>,
): AssociativeIdentity<ReadonlyMap<K, A>> => ({
  concat: makeUnionAssociative(E, S).concat,
  id: empty,
})
...
export const foldMap = <K>(
  O: Ord<K>,
): (<M>(M: AssociativeIdentity<M>) => <A>(f: (a: A) => M) => (m: ReadonlyMap<K, A>) => M) => {
...
export const foldMapWithIndex = <K>(
  O: Ord<K>,
): (<M>(M: AssociativeIdentity<M>) => <A>(f: (k: K, a: A) => M) => (m: ReadonlyMap<K, A>) => M) => {
  const keysO = keys(O)
  return (M) => (f) => (m) => {
    let out = M.id
    for (const k of keysO(m)) {
